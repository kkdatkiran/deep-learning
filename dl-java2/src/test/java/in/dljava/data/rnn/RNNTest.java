...
import in.dljava.recurrent.CustomSimpleRNNConf;
...
class RNNTest {
	...
	void test() throws Exception {
		...
		MultiLayerConfiguration conf = (new NeuralNetConfiguration.Builder()).updater(new Sgd()).list()
				.layer(new CustomSimpleRNNConf.Builder().nIn(28 * 28).nOut(256).activation(Activation.TANH).build())
				.layer(new CustomSimpleRNNConf.Builder().nIn(256).nOut(32).activation(Activation.TANH).build())
